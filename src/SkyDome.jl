...
function sky(scene; 
    ...
    if any(Idif .> 0.0)
        @assert ntheta > 0
        @assert nphi > 0
        sources = sky_dome(scene, Idif = convert_svector(Idif), nrays_dif = nrays_dif, 
...
"""
function radiosity(m::UniformSky, sky::SkySectors, Idif::SVector{nw, Float64} = SVector{1,Float64}(1.0)) where nw
    I = SVector{nw, Float64}[(
        t = (s.Φᵤ - s.Φₗ)*(cos(s.θₗ)^2 - cos(s.θᵤ)^2)/2/π;
        SVector{nw,Float64}(t.*Idif[i] for i in 1:nw)
            ) for s in sky]
    SkyDome(sky, I)
end
...
"""
function radiosity(m::StandardSky, sky::SkySectors, Idif::SVector{nw, Float64} = SVector{1,Float64}(1.0)) where nw
    I = SVector{nw, Float64}[(
        t = (s.Φᵤ - s.Φₗ)*((4cos(s.θₗ) + 3)*cos(s.θₗ)^2 - (4cos(s.θᵤ) + 3)*cos(s.θᵤ)^2)/14/π;
        SVector{nw,Float64}(t.*Idif[i] for i in 1:nw)
            ) for s in sky]
    SkyDome(sky, I)
end
...
function radiosity(m::CIE, sky::SkySectors, Idif::SVector{nw, Float64} = SVector{1,Float64}(1.0);
                    rtol=sqrt(eps(Float64)), atol=0.0, maxevals=typemax(Int)) where {nw}
    I = SVector{nw, Float64}[(
        t = hcubature(x -> radiance(m, x[1], x[2])*cos(x[1])*sin(x[1]),
