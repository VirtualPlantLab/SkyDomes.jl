...
function DirectionalSource(sky::SkyDome, mesh, nrays)
    ...
    [DirectionalSource(mesh, θ = θ[i], Φ = Φ[i], radiosity = sky.I[i],
...
function sky_dome(mesh; Idif = 1.0, nrays_dif = 1_000,
    ...
    sources = DirectionalSource(skydome, mesh, nrays_dif)
    return sources
end
...
function sky(mesh;
    ...
    if any(Idif .> 0.0)
        @assert ntheta > 0
        @assert nphi > 0
        sources = sky_dome(mesh, Idif = convert_svector(Idif), nrays_dif = nrays_dif,
    ...
    if any(Idir .> 0.0)
        source = DirectionalSource(mesh, θ = theta_dir, Φ = phi_dir,
