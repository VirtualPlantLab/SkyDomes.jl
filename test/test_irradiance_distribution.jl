...
let
    ...
    solid_standard = SkyDomes.radiosity(standard, solid)
    @test all(sum(solid_standard.I) .≈ [1.0])
    angles_standard = SkyDomes.radiosity(standard, angles)
    @test all(sum(angles_standard.I) .≈ [1.0])

    # Standard overcast distribution - 3 wavebands
    standard = StandardSky()
    solid_standard = SkyDomes.radiosity(standard, solid, threes)
    @test all(sum(solid_standard.I) .≈ [1.0, 1.0, 1.0])
    angles_standard = SkyDomes.radiosity(standard, angles, threes)
